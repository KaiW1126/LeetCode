...
nums.sort()            # インプレースソート（デフォルト昇順: 小さい順）
nums.sort(reverse=True) # 降順（大きい順）にソート
nums.pop()             # 末尾の要素を取り出して削除 O(1)
nums.pop(0)            # 先頭の要素を取り出して削除 O(n) ※遅いので注意
list(set(nums))        # 重複削除
...
8. **リストの `pop()` と計算量**: 末尾からの `.pop()` は O(1) だが、先頭からの `.pop(0)` は全要素がずれるため O(N) かかる点に注意
